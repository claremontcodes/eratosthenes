...
function longitude(jt) {
  const g = mean_anomaly(jt);
  return Q0 + Q1 * jt + L0 * Math.sin(g) + L1 * Math.sin(2 * g);
}
...
function right_ascension(jt) {
  const l = longitude(jt);
  return Math.atan2(Math.sin(l) * Math.cos(obliquity(jt)), Math.cos(l));
}
...
function sin_declination(jt) {
  return Math.sin(longitude(jt)) * Math.sin(obliquity(jt));
}
...
function hour_angle(jt, lon_deg) {
  return sidereal.local(jt, lon_deg) - right_ascension(jt);
}
...
function sin_elevation(jt, lat_deg, lon_deg) {
  const sin_lat = Math.sin(units.rad(lat_deg));
  const cos_lat = Math.cos(units.rad(lat_deg));
  const sin_dec = sin_declination(jt);
  const cos_dec = Math.sqrt(1 - sin_dec * sin_dec);
  const ha = hour_angle(jt, lon_deg);
  const cos_hour_angle = Math.cos(ha);
...
function elevation(jt, lat_deg, lon_deg) {
  return Math.asin(sin_elevation(jt, lat_deg, lon_deg));
}
...
function azimuth(jt, lat_deg, lon_deg) {
  ...
  const hour_angle = sidereal.local(jt, lon_deg) - right_ascension(jt);
